...
```
🐨 Next, <InlineFile file="tailwind.config.ts">Click here to create the `tailwind.config.ts` file</InlineFile> or initialize a config file by running this command in your terminal:

```bash
# cd playground directory
