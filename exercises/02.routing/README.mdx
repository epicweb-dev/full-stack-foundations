...
a new one that enables better collection of related files (which helps with
long-term maintenance).

We will actually be using a different convention for our app that is based on
the v2 convention but allows for even better collection of related files. The
